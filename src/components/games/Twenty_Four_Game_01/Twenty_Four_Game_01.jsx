...
import "./Twenty_Four_Game_01.css";
...
function generatePuzzle(mode) {
  if (mode === "mode2") return Array.from({ length: 4 }, () => Math.floor(Math.random() * 21));
  if (mode === "mode3") return Array.from({ length: 4 }, () => Math.floor(Math.random() * 41));
  return Array.from({ length: 4 }, () => Math.floor(Math.random() * 9) + 1);
}

/* Solver with ^ included */
  ...
  function helper(numbers, expressions) {
    ...
    for (let i = 0; i < numbers.length; i++) {
      for (let j = 0; j < numbers.length; j++) {
        ...
        if (operators["^"]) {
          if (Number.isFinite(Math.pow(a, b))) candidates.push([Math.pow(a, b), `(${exprA}^${exprB})`]);
          if (Number.isFinite(Math.pow(b, a))) candidates.push([Math.pow(b, a), `(${exprB}^${exprA})`]);
        }
        if (operators["!"]) {
...
/* =========================
   Component
   ========================= */
export default function Twenty_Four_Game_01({ onQuit }) {
  ...
  function newGame() {
    setConfig((prev) => ({ ...prev, customNumbers: generatePuzzle(prev.mode) }));
    setMessage("");
    setSolutionInput("");
    setSolutions([]);
  }
  ...
  function checkSolution() {
    try {
      ...
      const val = eval(expr);
      if (Math.abs(val - config.target) < 1e-6) {
  ...
  function toggleSolutions() {
    if (!showSolutions) {
      const sols = solveTarget(config.customNumbers, config.operators, config.target);
  ...
  return (
    <div className="tfour-container">
      ...
      <div className="tfour-buttons">
        ...
        <button style={btnStyle} onClick={() => setShowSetup(true)}>Setup</button>
      ...
      {showSetup && (
        <div className="tfour-box">
          ...
          <label>
            ...
            <select
              value={config.target}
              onChange={(e) => setConfig({ ...config, target: parseInt(e.target.value, 10) })}
            >
              {[24, 12, 18, 30, 36, 48].map((t) => (
          ...
          <label>
            ...
            <input
              ...
              onChange={(e) =>
                setConfig({ ...config, customNumbers: e.target.value.split(",").map((n) => parseInt(n.trim(), 10) || 0) })
              }
            />
          </label>
          <label>
          ...
          <label>
            ...
            <input
              ...
              onChange={(e) => setConfig({ ...config, operatorTextColor: e.target.value })}
            />
          </label>
          <label>
            ...
            <input
              ...
              onChange={(e) => setConfig({ ...config, operatorBgColor: e.target.value })}
            />
          </label>
          <br />
